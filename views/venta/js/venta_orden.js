        ...
        success: function (data) {
            ...
            if(data.id_tipo_pedido == 1){
                ...
                if($("#print_pre").val() == 1){ // impresion por comanda
                    ...
                    $('.btn-imp').html('<a href="'+$("#url").val()+'venta" class="btn btn-inverse" data-toggle="tooltip" data-placement="top" data-original-title="Monitor de mesas"><i class="fas fa-desktop"></i></a> <button  onclick="impPreCuenta('+data.id_pedido+','+data.id_mesa+',\''+data.estado_mesa+'\',true)" class="btn btn-inverse" data-toggle="tooltip" data-placement="top" data-original-title="Imprimir pre cuenta"><i class="fas fa-print"></i></button> <div class="btn-group" data-toggle="tooltip"><div class="dropdown-menu" id="list-comandas"></div></div>');                    
                }
                $('#cliente_id').val(1);
...
var calculo_efectivo = function(){
    var cal1 = parseFloat($(".totalPedido").val().replace(',', "")) - parseFloat($("#pago_tar").val().replace(/,/g, "")) - parseFloat($("#pago_yape").val().replace(/,/g, "")) - parseFloat($("#pago_plin").val().replace(/,/g, "")) - parseFloat($("#pago_tran").val().replace(/,/g, "")) ;
...
/* Pago efectivo */
$('#pago_efe,#pago_tar,#pago_yape,#pago_plin,#pago_tran').on('keyup', function(){
    calculo_efectivo();
});
...
$('#pago_tar').on('keyup', function(){
    calculo_total();
    $('.totalPedidoMenosTarjeta').text(($('.totalPedido').val() - $('#pago_tar').val()));
});
...
var ComTarj = function(thiss){

    var com_tar = $('#com_tar').val()/100;
    if($(thiss).prop('checked')){
        $('.display-comision-tarjeta').show();
        $(".comision_tarjeta").val((($('.totalPedido').val())*com_tar).toFixed(2));
        $('.comision_tarjeta').text((($('.totalPedido').val())*com_tar).toFixed(2));
...
$('#com_tarj').on('change', function(event){
    ComTarj(this);
