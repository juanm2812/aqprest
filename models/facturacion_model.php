...
class Facturacion_Model extends Model
{
	...
	public function Datos1()
    {
        try
        {
            ...
            $stm = $this->db->prepare("SELECT v.*,CASE WHEN v.estado = 'a' THEN IFNULL((v.total+v.comis_del-v.desc_monto),0) ELSE 0 END AS total FROM v_ventas_con AS v INNER JOIN v_caja_aper AS c ON v.id_apc = c.id_apc WHERE (DATE_FORMAT(v.fec_ven,'%Y-%m-%d') >= ? AND DATE_FORMAT(v.fec_ven,'%Y-%m-%d') <= ?) AND v.id_tdoc like ? AND v.id_tdoc <> 3 ".$estado." GROUP BY v.id_ven");
