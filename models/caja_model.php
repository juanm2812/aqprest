...
class Caja_Model extends Model
{
    ...
    public function monitor_ventas_list()
    {
        try
        {
            ...
            $stm = $this->db->prepare("SELECT *, CASE WHEN estado = 'a' THEN pago_efe+pago_tar ELSE 0 END monto_total FROM v_ventas_con WHERE (fec_ven >= ? AND fec_ven <= ?) AND id_tped like ? AND id_tdoc like ? ".$consulta."");
