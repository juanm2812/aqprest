...
class Informe_Model extends Model
{
    ...
    public function finanza_arq_resumen_default($data)
    {
        try
        {    
            $stm = $this->db->prepare("SELECT v.id_apc,v.id_ped,
            IFNULL(SUM(v.pago_efe),0) AS pago_efe, 
            SUM(CASE WHEN (v.id_tpag = 2 OR v.id_tpag = 3) THEN v.pago_tar ELSE 0 END) AS pago_tar,
            SUM(CASE WHEN v.id_tpag = 5 THEN v.pago_tar ELSE IFNULL((v.pago_yape),0) END) AS pago_yape,
            SUM(CASE WHEN v.id_tpag = 7 THEN v.pago_tar ELSE IFNULL((v.pago_tran),0) END) AS pago_tran,
            SUM(CASE WHEN v.id_tpag = 8 THEN v.pago_tar ELSE IFNULL((v.pago_plin),0) END) AS pago_plin,
            -- IFNULL(SUM(v.pago_yape),0) AS pago_yape, 
            -- IFNULL(SUM(v.pago_plin),0) AS pago_plin, 
            -- IFNULL(SUM(v.pago_tran),0) AS pago_tran, 
            IFNULL(SUM(v.desc_monto),0) AS descu, 
            IFNULL(SUM(v.comis_tar),0) AS comis_tar, 
            IFNULL(SUM(v.comis_del),0) AS comis_del, 
            IFNULL(SUM(v.pago_efe+v.pago_tar+v.pago_yape+v.pago_plin+v.pago_tran),0) AS total, v.estado FROM v_ventas_con AS v LEFT JOIN tm_pedido_delivery AS d ON v.id_ped = d.id_pedido WHERE v.id_apc = ? AND v.estado <> 'i' AND (d.id_repartidor IS NULL OR d.id_repartidor = 1 OR d.id_repartidor < 2000)");
    ...
    public function finanza_arq_imp($data)
    {
        ...
        {      
            ...
            $c->{'Principal'} = $this->db->query("SELECT v.id_apc,v.id_ped,IFNULL(SUM(v.pago_efe),0) AS pago_efe, IFNULL(SUM(v.pago_tar),0) AS pago_tar, IFNULL(SUM(v.desc_monto),0) AS descu, IFNULL(SUM(v.comis_tar),0) AS comis_tar, IFNULL(SUM(v.comis_del),0) AS comis_del, IFNULL(SUM(v.pago_efe+v.pago_tar+v.pago_yape+v.pago_plin+v.pago_tran),0) AS total, v.estado FROM v_ventas_con AS v LEFT JOIN tm_pedido_delivery AS d ON v.id_ped = d.id_pedido WHERE v.id_apc = {$data} AND v.estado <> 'i'")
            ->fetch(PDO::FETCH_OBJ);
            ...
            $c->{'Pagos'} = $this->db->query("SELECT X.DESCRIPCION,X.TRANSACCIONES,X.MONTO FROM(
                SELECT 'EFECTIVO' AS DESCRIPCION,
                CASE WHEN (b.id_tipo_pago = 1 OR b.id_tipo_pago = 3) AND (a.pago_efe>0 OR a.pago_efe_none>0)
                 THEN nvl((SELECT COUNT(1) FROM tm_venta c WHERE c.id_tipo_pago = 1 AND c.id_apc = a.id_apc),0)
                    + nvl((SELECT COUNT(1) FROM tm_venta c WHERE c.id_tipo_pago = 3 AND (c.pago_efe>0 OR c.pago_efe_none>0) AND c.id_apc = a.id_apc),0)
                END TRANSACCIONES, 
                CASE WHEN (b.id_tipo_pago = 1 OR b.id_tipo_pago = 3) AND (a.pago_efe>0 OR a.pago_efe_none>0)
                  THEN nvl((SELECT sum(c.pago_efe) FROM tm_venta c WHERE c.id_tipo_pago = 1 AND c.id_apc = a.id_apc),0)
                    + nvl((SELECT sum(c.pago_efe) FROM tm_venta c WHERE c.id_tipo_pago = 3 AND (c.pago_efe>0 OR c.pago_efe_none>0) AND c.id_apc = a.id_apc),0)
                END MONTO
                FROM tm_venta a, tm_tipo_pago b 
                 WHERE a.id_tipo_pago = b.id_tipo_pago AND a.id_apc = {$data}
                 AND (b.id_tipo_pago = 1 OR b.id_tipo_pago = 3) AND (a.pago_efe>0 OR a.pago_efe_none>0)
                UNION  
                SELECT 'TARJETA' AS DESCRIPCION,
                CASE WHEN (b.id_tipo_pago = 2 OR b.id_tipo_pago = 3) AND a.pago_tar>0 
                 THEN nvl((SELECT COUNT(1) FROM tm_venta c WHERE c.id_tipo_pago = 2 AND c.id_apc = a.id_apc),0)
                    + nvl((SELECT COUNT(1) FROM tm_venta c WHERE c.id_tipo_pago = 3 AND c.pago_tar>0 AND c.id_apc = a.id_apc),0)
                END TRANSACCIONES, 
                CASE WHEN (b.id_tipo_pago = 2 OR b.id_tipo_pago = 3) AND a.pago_tar>0 
                  THEN nvl((SELECT sum(c.pago_tar) FROM tm_venta c WHERE c.id_tipo_pago = 2 AND c.id_apc = a.id_apc),0)
                    + nvl((SELECT sum(c.pago_tar) FROM tm_venta c WHERE c.id_tipo_pago = 3 AND c.pago_tar>0  AND c.id_apc = a.id_apc),0)
                END MONTO
                FROM tm_venta a, tm_tipo_pago b 
                 WHERE a.id_tipo_pago = b.id_tipo_pago  AND a.id_apc = {$data}
                AND (b.id_tipo_pago = 2 OR b.id_tipo_pago = 3) AND a.pago_tar>0
                UNION
                SELECT 'YAPE' AS DESCRIPCION,
                CASE WHEN (b.id_tipo_pago = 5 OR b.id_tipo_pago = 3) AND (a.pago_tar > 0 OR a.pago_yape > 0 )
                 THEN nvl((SELECT COUNT(1) FROM tm_venta c WHERE c.id_tipo_pago = 5 AND c.id_apc = a.id_apc),0)
                    + nvl((SELECT COUNT(1) FROM tm_venta c WHERE c.id_tipo_pago = 3 AND c.pago_yape>0 AND c.id_apc = a.id_apc),0)
                END TRANSACCIONES, 
                CASE WHEN (b.id_tipo_pago = 5 OR b.id_tipo_pago = 3) AND (a.pago_tar > 0 OR a.pago_yape > 0 ) 
                  THEN nvl((SELECT sum(c.pago_tar) FROM tm_venta c WHERE c.id_tipo_pago = 5 AND c.id_apc = a.id_apc),0)
                    + nvl((SELECT sum(c.pago_yape) FROM tm_venta c WHERE c.id_tipo_pago = 3 AND c.pago_yape>0  AND c.id_apc = a.id_apc),0)
                END MONTO
                FROM tm_venta a, tm_tipo_pago b 
                 WHERE a.id_tipo_pago = b.id_tipo_pago  AND a.id_apc = {$data}
                AND (b.id_tipo_pago = 5 OR b.id_tipo_pago = 3) AND (a.pago_tar > 0 OR a.pago_yape > 0 )
                UNION
                SELECT 'PLIN' AS DESCRIPCION,
                CASE WHEN (b.id_tipo_pago = 8 OR b.id_tipo_pago = 3) AND (a.pago_tar > 0 OR a.pago_plin > 0 )
                 THEN nvl((SELECT COUNT(1) FROM tm_venta c WHERE c.id_tipo_pago = 8 AND c.id_apc = a.id_apc),0)
                    + nvl((SELECT COUNT(1) FROM tm_venta c WHERE c.id_tipo_pago = 3 AND c.pago_plin>0 AND c.id_apc = a.id_apc),0)
                END TRANSACCIONES, 
                CASE WHEN (b.id_tipo_pago = 8 OR b.id_tipo_pago = 3) AND (a.pago_tar > 0 OR a.pago_plin > 0 )
                  THEN nvl((SELECT sum(c.pago_tar) FROM tm_venta c WHERE c.id_tipo_pago = 8 AND c.id_apc = a.id_apc),0)
                    + nvl((SELECT sum(c.pago_plin) FROM tm_venta c WHERE c.id_tipo_pago = 3 AND c.pago_plin>0  AND c.id_apc = a.id_apc),0)
                END MONTO
                FROM tm_venta a, tm_tipo_pago b 
                 WHERE a.id_tipo_pago = b.id_tipo_pago  AND a.id_apc = {$data}
                AND (b.id_tipo_pago = 8 OR b.id_tipo_pago = 3) AND (a.pago_tar > 0 OR a.pago_plin > 0 )
                UNION
                SELECT 'TRANFERENCIA' AS DESCRIPCION,
                CASE WHEN (b.id_tipo_pago = 7 OR b.id_tipo_pago = 3) AND (a.pago_tar > 0 OR a.pago_tran > 0 )
                 THEN nvl((SELECT COUNT(1) FROM tm_venta c WHERE c.id_tipo_pago = 7 AND c.id_apc = a.id_apc),0)
                    + nvl((SELECT COUNT(1) FROM tm_venta c WHERE c.id_tipo_pago = 3 AND c.pago_tran>0 AND c.id_apc = a.id_apc),0)
                END TRANSACCIONES, 
                CASE WHEN (b.id_tipo_pago = 7 OR b.id_tipo_pago = 3) AND (a.pago_tar > 0 OR a.pago_tran > 0 )
                  THEN nvl((SELECT sum(c.pago_tar) FROM tm_venta c WHERE c.id_tipo_pago = 7 AND c.id_apc = a.id_apc),0)
                    + nvl((SELECT sum(c.pago_tran) FROM tm_venta c WHERE c.id_tipo_pago = 3 AND c.pago_tran>0  AND c.id_apc = a.id_apc),0)
                END MONTO
                FROM tm_venta a, tm_tipo_pago b 
                 WHERE a.id_tipo_pago = b.id_tipo_pago  AND a.id_apc = {$data}
                AND (b.id_tipo_pago = 7 OR b.id_tipo_pago = 3) AND (a.pago_tar > 0 OR a.pago_tran > 0 )
                UNION
                SELECT b.DESCRIPCION, 
                            nvl(COUNT(b.id_tipo_pago),0)  TRANSACCIONES, 
                           nvl(sum(a.pago_tar),0)  MONTO FROM tm_venta a, tm_tipo_pago b 
                            WHERE a.id_tipo_pago = b.id_tipo_pago AND a.id_tipo_pago NOT IN (1,2,3,5,7,8) AND a.id_apc = {$data}
                ) X WHERE X.DESCRIPCION IS NOT NULL GROUP BY X.DESCRIPCION")
                            ->fetchAll(PDO::FETCH_OBJ);             
            ...
            $c->{'ComisionTarjeta'} = $this->db->query("SELECT SUM(IFNULL((comis_tar),0)) AS total, COUNT(*) AS cant FROM v_ventas_con  WHERE id_apc = {$data} AND estado = 'a' AND comis_tar > 0")
            ->fetch(PDO::FETCH_OBJ);     
